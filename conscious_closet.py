...
google_places = GooglePlaces(os.environ["PLACES_API_KEY"])
...
def weather_suggestions():
    ...
    if city:
        try:
            response = requests.get(BASE_URL, params={'q': city, 'appid': weather_key, 'units': 'imperial'})        
...
def get_weather_forecast(city, day):
    response = requests.get(FORECAST_URL, params={'q': city, 'appid': weather_key, 'units': 'imperial'})        
